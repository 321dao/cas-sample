...
var loginURL = "http://localhost:7070/cas/login?service=%s"
var validateURL = "http://localhost:7070/cas/validate?service=%s&ticket=%s"
	...
	if user == "" {
		...
		if ticket != "" {
			...
			if spli[0] == "yes" {
				...
				c.Redirect(fmt.Sprintf(loginURL, serviceURL), 303)
				return
			}
		} else {
...
func main() {
	...
	iris.Get("/logout", func(c *iris.Context) {
		DeleteUser(c)
		c.Redirect("http://localhost:7070/cas/logout?service=http://localhost:8080/", 303)
	})
